...
def getShuffledPlants(smallest_set=359):
    ...
    for data_dir in data_dirs:
        add_image_filepaths ,add_label_filepaths ,add_color_filepaths ,add_meta_filepaths = my_modules.import_dataset_filepaths \
    ...
    for (new_plant_img, new_plant_lbl, new_plant_col_lbl, new_stem) in new_plant_data_bundle_list:
            ...
            for try_num in range(MAX_TRIES):
                ...
                for try_d in np.arange(dmax / POINTS_ON_LINE, dmax, dmax / POINTS_ON_LINE):
                    ...
                    if foundGoodSpot:
                        ...
                        cv2.circle(stem_mask,(xmin + int(new_stem["x"]), ymin + int(new_stem["y"])), 11, 1, thickness=-1)
                        #stem_mask[ymin + int(new_stem["y"]), xmin + int(new_stem["x"])] = 1
        ...
        if not (foundGoodSpot):  # place randomly
            ...
            if foundGoodSpot:
                ...
                cv2.circle(stem_mask, (rand_xmin + int(new_stem["x"]), rand_ymin + int(new_stem["y"])), 11, 1, thickness=-1)
                #stem_mask[rand_ymin + int(new_stem["y"]), rand_xmin + int(new_stem["x"])] = 1
